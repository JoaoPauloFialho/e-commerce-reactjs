...
export const useCarrinhoContext = () => {
    ...
    function adicionarProdutoNoCarrinho(produtoNovo) {
        ...
        if (!temNoCarrinho) {
            ...
            setCarrinho([...carrinho, produtoNovo])
        }
    ...
    function removerProdutoDoCarrinho(produtoRemover) {
        ...
        if (produtoRemover.quantidade === 1) {
            localStorage.setItem('carrinho', JSON.stringify(carrinho.filter(produto => produto.id !== produtoRemover.id)))
    ...
    function mudarQuantidade(id, qnt) {
        let novoCarrinho = JSON.parse(localStorage.getItem('carrinho'))
        localStorage.setItem('carrinho', JSON.stringify([...novoCarrinho.map(
            produto => {
